...
**Status:** Technical specification + implementation provenance

---

## Abstract

State-of-the-art deep learning models increasingly leverage non-linear routing mechanisms — branching gradient paths, multi-head routing, mixture-of-experts. While standard diagonal adaptive optimizers (AdamW, Sophia) scale efficiently, they are structurally bottlenecked by a foundational assumption: that the loss landscape's principal axes of curvature align with the parameter space's standard coordinate basis. When that assumption fails, diagonal estimators misjudge sharp and flat directions, producing gradient oscillation, step-size stagnation, or divergence.

This paper introduces **RACASO** (Rotation-Aligned Cautious Approximately Second-Order Optimizer), an optimization framework that bridges Kronecker-factored structural preconditioning and second-order curvature alignment. RACASO dynamically projects each 2-D parameter into a Kronecker-factored eigenbasis (the privileged basis where its gradient covariance is approximately diagonalized), computes an unbiased per-element diagonal Hessian estimate within that basis via Hutchinson's trace estimator on a Pearlmutter-style HVP, and applies a Sophia-style per-element cautious clip on the resulting rotated update.

Two curvature-estimator paths are shipped: Hutchinson HVP (the true Hessian diagonal, including negative-curvature directions) and Gauss-Newton-Bartlett (the positive-semidefinite GN approximation, a structurally bulletproof fallback). A four-layer safety chain (eigh residual gate, RAdam cold-start gate, 1-D Yogi fallback, spread cap on rotated row norms) plus an L5 safe-skip on non-finite HVP output keeps the optimizer numerically stable across failure modes that competing second-order optimizers (SOAP, Muogi-class NS-orthogonalization) cannot survive.

We document the eight-attempt diagnostic provenance of the Hutchinson HVP integration with PyTorch eager autograd, the operator-shape rules that determine which forward-graph operators are second-derivative-safe, and the conditions under which RACASO's L5 safe-skip triggers — preserving the failure record so consumers can audit whether their own forward graph satisfies the second-derivative-bounded conditions RACASO requires for full design-intent operation.
...
This formulation assumes the Hessian $\mathcal{H}$ is approximately diagonal — that each parameter varies independently of all others.

In modern architectures the assumption is cleanly falsified. Branching gradient paths, weight sharing, multi-head attention, and joint-normalized fusion sites introduce off-diagonal dependencies. Geometrically, this manifests as sharp asymmetric loss valleys running diagonally across the standard coordinate axes. A diagonal optimizer encountering a diagonal valley crosses back and forth over the ridges rather than traveling smoothly down the floor.

Matrix-preconditioned optimizers (SOAP, Shampoo) achieve superior convergence not because their gradient proxies are inherently more precise, but because they rotate the gradients into a structural eigenbasis where the local coordinates are temporarily uncoupled. However, both rely entirely on first-order gradient history ($g_t \cdot g_t^T$) to build that basis, which is structurally hostile to coupled-gradient regimes — when one factor's gradient distribution shifts abruptly, the Kronecker covariance accumulates rank-1 outer products that produce near-singular factors, and eigh either returns silently-NaN eigenvectors or non-trustworthy rotations.

RACASO addresses both: maintain SOAP's rotation-into-privileged-basis property, but estimate the per-element step size in that basis using true second-order curvature (Hutchinson HVP) rather than gradient-squared proxies. Add a per-element cautious clip in the rotated basis to bound steps regardless of curvature estimate quality. Add safety layers around the rotation refresh, the cold-start regime, and the second-derivative failure surface that PyTorch eager autograd exposes.

---

## 2. The Architecture of RACASO

For a 2-D matrix parameter $W \in \mathbb{R}^{d_{out} \times d_{in}}$, RACASO combines Kronecker-factored axis rotations with Hessian-vector products, then applies Sophia's cautious step in the rotated basis. Visually:
...
The orthogonal matrices $U_L$ and $U_R$ form the **privileged basis**. A progressive-ridge `_safe_eig_with_residual` helper retries the eigendecomposition at increasing ridge scales `(0, 1e-6, 1e-3, 1e-1)` if the bare-input form returns NaN columns (a known PyTorch failure mode for near-singular symmetric inputs). The L2 safety layer then gates acceptance: each side's new eigenvectors are committed independently only if the reconstruction residual $\|M - U \Lambda U^T\|_F$ is below threshold and isfinite. A bad eigh on the L side does not reject the R side's good refresh.
...
**Cost:** one extra backward through the X-subgraph per refresh step (the Pearlmutter HVP shares the live forward graph). On non-refresh steps, zero overhead — the optimizer uses the previously-stored $U_L$, $U_R$, $\tilde{h}_t$.

#### 2.2.2 Gauss-Newton-Bartlett (GNB) — the bulletproof fallback

When Hutchinson HVP can't run (or is structurally hostile to a given forward graph — see §6), GNB replaces the Hessian with the Gauss-Newton approximation:
...
### 2.4 Spread cap + Projection back to parameter space
...
where $r_t$ is the RAdam rectification factor (see §3) — bounded in $[0, 1]$ and equal to 0 during cold-start, equal to the RAdam-standard variance correction post-warmup.
...
## 3. The Four-Layer Safety Chain + L5

RACASO's stability profile depends on five independent safety mechanisms layered through the `step()` body. Each addresses a specific failure mode; together they form the contract that distinguishes RACASO from naïve Sophia-with-rotation.

| Layer | Mechanism | What it catches | Failure action |
|---|---|---|---|
| **L1** | Spread cap on rotated update row norms | Pathological per-eigendirection step spreads after Sophia clip | Soft degradation — row norms equalized to ≤ spread_cap ratio |
...
| **L3** | Vanilla Yogi fallback for 1-D params + missing HVP stash | Norms, biases, scalars; non-refresh steps where the stash is absent | Vanilla Yogi update; `hessian_diag_rot` preserves its prior EMA |
...
| **L5** | Non-finite HVP gate on the batched second backward | Forward-graph operators whose second derivative is unbounded over visited input range (see §6) | Skip stash this refresh; `hessian_diag_rot` carries to next refresh via EMA; counter tracked for telemetry |
...
**Batched HVP via Pearlmutter (the speed fix).** Naïve per-parameter HVP would call `autograd.grad(g_live_i, p_i, grad_outputs=z_i)` for each parameter. Each call walks the retained second-order graph independently — with 16 parameters that's 16× the graph traversal cost. The Pearlmutter identity collapses it to one:
...
Mathematically identical to the per-param loop; cost drops 16× → 1×. This is the standard trick in Sophia / Adahessian-style HVP implementations.
...
Matrix-orthogonalization setups like Muon and RAMuogi apply Newton-Schulz polynomials to force gradient matrices into orthogonal spaces. The fifth-order Jordan polynomial is the standard choice; convergence requires the input matrix's spectral norm to lie in a specific band. When branching-path architectures induce dynamic gradient distribution shifts, the gradient matrix becomes ill-conditioned (condition proxies scaling over 200 in practice). The polynomial diverges, the optimizer's convergence-check safety layer triggers, and the update is rejected.

The RAMuogi paper (MetaforeAI/Muogi, §5) documents this failure mode: under a joint-normalized branching-fusion architecture, NS5 fell back to a vanilla Yogi update on ~90% of steps. Numerically RAMuogi survived; the optimizer's design-intent orthogonalization rarely engaged.

RACASO does not orthogonalize the gradient. It uses the Kronecker eigenvectors strictly as a coordinate-transformation mechanism — projecting into the basis, applying the cautious step there, projecting back. If a branching path introduces a sudden surge of ill-conditioned curvature, the L1 spread cap absorbs the spread, the L2 eigh safe-skip handles the rotation refresh failure, and the trajectory continues. The optimizer's design-intent rotation continues to fire on every step (the rotation matrices are stable across refreshes, only updated when the eigh refresh succeeds), and the cautious clip absorbs the magnitude shock.
...
### 5.3 Honest claim about L5 — what RACASO actually does and does not do

An earlier draft of this paper claimed RACASO "never skips updates." That is not true and was never true; L5 was added during the integration provenance documented in §6 specifically because forward graphs in real architectures contain operators whose second derivative is unbounded over the input range the model visits during training. When the batched HVP backward returns non-finite Hz values for second-derivative-numerical reasons, RACASO **does** skip the curvature-estimate update for that refresh — `hessian_diag_rot` carries via its prior EMA, the rotated-basis update for that refresh proceeds with stale curvature, and the optimizer counter `_l5_skip_count` increments for telemetry.

The skip is graceful (training continues, loss descends, parameter updates still apply) and the contract is well-defined: the failure surface is identified, located, and the user can either (a) accept the L5 absorption rate as background noise, (b) audit the forward graph for the operator-shape rules in §6 and replace the offending operator with a bounded-second-derivative equivalent, or (c) switch to the GNB strategy (`hvp_strategy="gnb"`) which avoids second derivatives entirely. RACASO ships all three paths; the user picks based on their forward graph's structure.

**Under conditions:** when the forward graph contains operators with bounded second derivatives over visited inputs, RACASO's L5 fires 0% of refreshes and delivers full design-intent compute. **We solved this** in our reference integration by identifying the offending operator (a `tensor.norm()` call inside a cosine attention computation) and replacing it with a bounded-second-derivative equivalent (negative squared Euclidean distance), bringing the L5 rate from ~12% to 0%. **Candidates we considered:**

1. **Bump the user-side eps** on the dividing expression $x / (\|x\| + \epsilon)$. This would protect the first derivative but not the second (the eps is on the wrong side of the autograd boundary; `tensor.norm()` computes its gradient internally without access to the user's floor). Tested, no effect.
...
The chosen fix is reference-architecture-specific (your project may have different reasons to keep cosine attention) but the operator-shape rules in §6 generalize across any project's choice.

---

## 6. The Second-Derivative DivBackward0 Failure Class

This section documents the class of forward-graph failure that L5 absorbs. It is independent of RACASO and applies to any second-order-aware optimizer (Hutchinson HVP, Gauss-Newton, K-FAC, Shampoo+second-moment-correction, Adahessian).
...
- The scalar formed from those gradients (e.g. $\sum_i \langle g_i, z_i \rangle$ for Hutchinson) is finite.
- The second-order backward (e.g. `autograd.grad(scalar, params)` for the batched HVP form) returns tensors that are uniformly NaN, with `isfinite_count = 0`.
...
The $1/\|x\|^3$ term in the second derivative is the failure surface. When $\|x\|$ is small — say $0.05$ — the second-derivative scale is $\sim 8000$. When $\|x\| \sim 0.01$, $\sim 10^6$. The Pearlmutter HVP traversal multiplies these terms with adjacent gradient components; the products hit float32 overflow → inf → NaN, and the NaN propagates through the entire second-order graph to every downstream parameter's $Hz$ output uniformly.
...
PyTorch ships the right tool, but the API has a subtle correctness requirement. `torch.autograd.set_detect_anomaly(True)` must be **active during the forward pass** for the eventual NaN-on-backward to include the forward Python stack trace identifying the offending operator's `file:line`. The usage pattern is:
...
The global-flag form (not the context-manager form) keeps anomaly mode active across both the forward and the subsequent HVP backward. Cost: ~5-10× slower forward. Arming for one refresh step is sufficient; the diagnostic fires once and identifies the exact source.
...
- Linear, Convolution (matmul / conv produce constant second derivatives)
...
The pattern: any operator whose second derivative contains a power of $1/x$ or $1/\|x\|$ where $x$ can approach zero is hazardous. The fix is either (a) replace the operator with a bounded-second-derivative equivalent (e.g. squared distance instead of cosine similarity for "which is closest" queries), or (b) floor-clamp the operator's argument **inside** the operator's input (e.g. `(x² + eps).sqrt()` instead of `x.sqrt() + eps`).

### 6.4 Implications for second-order optimization adoption

Codebases written for first-order optimization may contain `tensor.norm()` in cosine attention, log-likelihood losses with small probabilities, or division-by-computed-magnitudes that work cleanly under SGD/Adam/AdamW but fail under HVP. The user-facing failure mode usually appears as "optimizer produces NaN gradients" or "loss diverges around step N." The actual cause is often an operator-shape mismatch between the forward graph and the optimizer's second-order traversal.

The right shape for the optimizer is to provide a diagnostic mode (anomaly trace + class flag) the user can flip on for one step to identify the offending operator. The right shape for the codebase is to audit forward-graph operators for second-derivative boundedness during migration to a second-order optimizer. RACASO's L5 absorbs the failure when it occurs, the diagnostic flag identifies the source, the rest is at the operator definition site, not the optimizer.

---

## 7. Engineering Provenance — Eight Attempts to Integrate Hutchinson with PyTorch Eager Autograd

This section is preserved verbatim from the integration log so future optimizer work has the failure record. Each attempt failed in a structurally specific way; each fix made the next failure visible. The lesson is preserved alongside the survivor: **each wall was a property of the codebase + PyTorch eager-autograd interaction, not a property of Hutchinson itself.** The fix at each wall was either small or out-of-scope-but-named. The recipe didn't survive contact with the codebase, but each contact taught the codebase what it needed.
...
| 6 | `linalg.eigh` returns NaN columns silently on near-singular input | PyTorch's `linalg.eigh` does not raise on near-rank-deficient matrices; it returns eigenvectors with NaN columns for the null space. The L2 layer's residual-threshold gate accepted these because `nan < threshold` is False but `max(nan, x)` is order-dependent — the broken Q could slip through | Explicit `isfinite()` check on Q + eigenvalues; progressive ridge cascade if non-finite; per-side independent Q commit (so a bad eigh on L doesn't reject R's good refresh) |
| 7 | Per-param HVP loop is 16× graph traversal cost | Naïve implementation calls `autograd.grad(g_live_i, p_i, grad_outputs=z_i)` per param. At 16 parameters, 16× the graph traversal. Pearlmutter batched HVP via $\sum_i \langle g_i^{\text{live}}, z_i \rangle$ collapses to 1× | Cost dropped 16×; HVP wall-clock from ~27 s/step to ~6 s/step at K=1 |
...
| `hvp_strategy` | `"hutchinson"` | Curvature estimator — `"hutchinson"` (true diagonal, incl. negative curvature) or `"gnb"` (Gauss-Newton fallback, positive semidefinite, no second derivatives) |
...
The optimizer is designed for failure modes that competing second-order methods do not survive: ill-conditioned Kronecker covariance under coupled-gradient regimes, Newton-Schulz convergence failure under sudden distribution shifts, and the silent failure modes PyTorch eager autograd exposes when the second-derivative path is naively integrated. The four-layer safety chain (L1-L4) absorbs the first three; the L5 safe-skip absorbs the fourth, documented in §6 as a general class for any second-order-aware optimizer.

The engineering provenance in §7 is preserved because each wall was a property of the codebase + framework interaction, not a property of Hutchinson itself. The fix at each wall was either small or out-of-scope-but-named. The recipe didn't survive contact with the codebase, but each contact taught the codebase what it needed. The diagnostic infrastructure built during that integration (stage trap, anomaly mode integration, site probe utility) is reusable for any future optimizer integration that hits the same operator-shape failure surface.

RACASO ships as a drop-in for any model with branching gradient paths and coupled-gradient regimes. The underlying composition (CASPR rotation + Sophia cautious step in the rotated basis, with the four-layer safety chain and L5 absorption) is general.
