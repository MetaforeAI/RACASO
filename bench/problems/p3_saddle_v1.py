...
class P3aSaddle2D(BenchProblem):
    ...
    # ARCHIVE COPY: live canonical version is in p3_saddle.py. P3a stays
    # as a 2-D vector problem (intrinsically tiny — see paper §8 note),
    # but the archive's name field is blanked so only one registration
    # is active.
    name = ""
    max_steps = 2000
...
class P3bSaddleN20(BenchProblem):
    ...
    # ARCHIVE: legacy 1-D vector version of P3b. Replaced by the
    # matrix-version (5x4) in p3_saddle.py.
    name = ""
    max_steps = 2000
    converged_tol = 0.0
    saddle: bool = True
