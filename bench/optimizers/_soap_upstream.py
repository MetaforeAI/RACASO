...
Vendored verbatim from https://github.com/nikhilvyas/SOAP (MIT
License, © 2024 Nikhil Vyas). Reference: Vyas et al. 2024,
"SOAP: Improving and Stabilizing Shampoo using Adam"
(arXiv:2409.11321).

This file is the upstream's `soap.py` reproduced unchanged for
provenance. The thin wrapper in ``bench/optimizers/soap.py``
re-exports the `SOAP` class for bench-harness use.
"""
import torch
...
class SOAP(optim.Optimizer):
    ...
    def init_preconditioner(self, grad, state, precondition_frequency=10, 
        ...
        # NOTE: dtype=grad.dtype added (bench harness uses float64 on
        # CPU). Upstream allocated default float32, which broke lerp_
        # against float64 grads.
        if grad.dim() == 1:
