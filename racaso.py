...
## Why RACASO for the X organ specifically

Under Morpheus v17.1's joint-norm path at DIH path B, the X organ's
gradient covariance is *coupled* across rows and columns via the
shared joint-norm denominator. SOAP's Kronecker assumption
(Σ ≈ Σ_L ⊗ Σ_R, row dependencies independent of column dependencies)
is violated by this coupling — eigh refresh on ``GG_L``/``GG_R`` hits
progressively ill-conditioned matrices and the fallback chain
eventually collapses. RAMuogi handled the coupling numerically but
